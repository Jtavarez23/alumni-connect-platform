...
-- Based on AC-ARCH-002a/002b specifications and existing implementation gaps
...
-- =============================================
-- 5. NOTIFICATION & ACTIVITY TABLES RLS POLICIES
-- =============================================
...
-- activity_feed table: Complete policies
DO $$
BEGIN
  ALTER TABLE public.activity_feed ENABLE ROW LEVEL SECURITY;
  
  -- View activities from connections and public content
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'activity_feed_select_relevant' AND tablename = 'activity_feed') THEN
    CREATE POLICY activity_feed_select_relevant ON public.activity_feed
      FOR SELECT USING (
        user_id = auth.uid() OR
        visibility = 'public' OR
        (visibility = 'connections' AND EXISTS (
          SELECT 1 FROM public.connections c 
          WHERE (c.user_id = auth.uid() AND c.connection_id = activity_feed.user_id AND c.status = 'accepted') OR
                (c.user_id = activity_feed.user_id AND c.connection_id = auth.uid() AND c.status = 'accepted')
        ))
      );
  END IF;
END$$;
