-- Multi-School Database Migration - Phase 1: Database Structure Enhancement
...
CREATE TABLE public.school_history (
  ...
  CONSTRAINT single_primary_per_user EXCLUDE USING gist (user_id WITH =) WHERE (is_primary = true)
);

-- 6. Create school verification levels
...
-- 7. Create cross-school events table
...
-- 8. Create school invitations system
...
CREATE INDEX idx_school_history_primary ON public.school_history(user_id) WHERE is_primary = true;
