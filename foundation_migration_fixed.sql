-- Alumni Connect - Foundation Migration (Fixed)
-- Create basic tables without vector dependency
...
-- Create page_faces for face detection (WITHOUT vector embedding for now)
CREATE TABLE IF NOT EXISTS public.page_faces (
  id uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  page_id uuid REFERENCES public.yearbook_pages(id) ON DELETE CASCADE,
  bbox integer[] NOT NULL,
  -- embedding will be added later when pgvector is available
  embedding_data jsonb, -- temporary storage for face embeddings
